...
function VerticalCarouselUp({
  ...
  // keep speed current without recreating callbacks
  useEffect(() => {
  ...
  const step = () => {
    ...
    if (a && b) {
      boostRef.current += (1 - boostRef.current) * 0.08; // ease boost back to 1
  ...
  // Preload images; only then start animation & reveal
  useLayoutEffect(() => {
    let cancelled = false;
    ...
    preloadAll.then(() => {
      ...
      // init animation after images are ready
      yRef.current = 0;
